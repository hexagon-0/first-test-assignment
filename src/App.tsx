  ...
  return (
    <div className="h-screen flex justify-center items-center">
      <div className="py-2.5 rounded-md shadow-shape ring-1 ring-inset ring-[#EEEEEE]">
        <CheckItem id="check-item-all">All pages</CheckItem>

        <Separator />

        <div className="h-[164px] overflow-y-scroll no-scrollbar">
          {pages.map((text, i) => (
            <CheckItem id={`check-item-${i}`} key={i} nOffset={true}>
              {text}
            </CheckItem>
          ))}
        </div>
...
function CheckItem ({ id, children, nOffset = false }: CheckItemProps) {
  return (
    <label htmlFor={id} className={`w-[370px] ${nOffset ? 'relative top-[-1px] left-[-1px]' : ''} flex justify-between items-center pt-2.5 pb-[9px] px-[22px] cursor-pointer`}>
      ...
      <input id={id} type="checkbox" className="w-[23px] h-[23px] relative left-[1px] border border-line-tone-faint rounded-md bg-center bg-no-repeat bg-none hover:bg-check hover:border-[#BDBDBD] active:bg-check-dark focus:ring-0 focus:ring-offset-0 active:ring active:ring-offset-0 active:ring-faint-blue checked:bg-check-white checked:bg-[#2469F6] checked:focus:bg-[#2469F6] checked:active:bg-[#2469F6] checked:bg-auto checked:hover:bg-check-white checked:hover:bg-[#5087F8] checked:active:bg-check-white" />
    </label>
  );
}
